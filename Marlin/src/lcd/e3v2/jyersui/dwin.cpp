 ...
 // process = File;
  ...
  if (card.isMounted() && !removed) {
    LOOP_L_N(i, _MIN(card.get_num_Files() + 1, TROWS))
      Draw_SD_Item(i, i, onlyCachedFileIcon);
  }
  else {
...
void CrealityDWINClass::Popup_Handler(PopupID popupid, bool option/*=false*/) {
  ...
  switch (popupid) {
    ...
    case ConfirmStartPrint: Draw_Popup(F("Loading Preview..."), F("Please visit"), F("github.com/Jyers/Marlin"), Popup); break;
    default: break;
  }
}
    ...
    if (selection == 0) {
      ...
      if (card.flag.filenameIsDir) {
        ...
        if (has_preview) {
          ...
          DWIN_ICON_Show(ICON, ICON_Preview, 28, 60); 
          gcode.process_subcommands_now(F("M117 Preview not found. Please install Slicer Scripts.")); 
        }
       #else
